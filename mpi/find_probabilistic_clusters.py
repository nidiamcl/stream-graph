...
def findProbabilisticClusters(network, nodes, csr_matrixg, fps, save_path, similarity='jaccard', threshold=0.05):
    ''' 
    input
        nodes: list of nodes
    ...
    for ri,node in enumerate(nodes):
        ...
        #     f.write("{},{},{},{}\n".format(node, threshold, len(passing_scores), len(passing_fps_idx)))
        ...
        if len(passing_scores) != 0:
            ...
            for i,fp in enumerate(zip(passing_fps_idx, passing_scores)):
                ...
                #     fi.write("{},{},{},{},{}\n".format(node, threshold, sum_of_passing_scores, fp_idx, fp[1]))  
        ...
        # no similar fingerprints, creating a new one
            ...
            print('outlier node:', node, 'sum nodes', sum_nodes)
...
def mergeProbabilisticFingerprints(fps_temp, fmap_temp, outliers_action, save_path, similarity='dotsim', threshold=0.3):
    ...
    for i, fp in enumerate(fps):
        ...
        # if the fingerprint was not merged (processed), and it's also not an outlier
